...
//        String resultJsonUserName = null;
//        for(int i=0;i<jsonArray.length();i++){
//            JSONObject ob = (JSONObject) jsonArray.get(i);
//            jsonUserName = ob.getString("userName");
//        }
//        for(int j=0;j<resultJson.length();j++){
//            JSONObject ob = (JSONObject) jsonArray.get(j);
//            resultJsonUserName = ob.getString("userName");
//        }
        System.out.println("获取用户信息:"+jsonArray.toString());
        System.out.println("调用接口获取用户信息:"+resultJson.toString());
        Assert.assertEquals(jsonArray.toString(),resultJsonArray.toString());
    }
    ...
    private JSONArray getJsonResult(GetUserInfoCase getUserInfoCase) throws IOException {
        ...
        List resultList = Arrays.asList(result);  //转化list
